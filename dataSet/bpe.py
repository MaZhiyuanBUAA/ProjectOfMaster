...
def bpe(path,save2,num_merges=30000):
  f = open(path,"rb")
  vocab_init = [ele.split("<!>") for ele in f.readlines()]
  vocab_init = [(v,int(freq)) for v,freq in vocab_init]
  start_time = time.time()
  #vocab_init = {'l o w *' : 5, 'l o w e r *' : 2,'n e w e s t *':6, 'w i d e s t *':3}
  vocab_init = defaultdict(int,vocab_init)
  ...
  print('sorted ....')
  vocab = [v for v,freq in vocab_item[:num_merges]]
  f = open('vocab_%d.txt' % num_merges, 'w')
